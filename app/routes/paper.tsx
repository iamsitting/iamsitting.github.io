...
export default function Paper() {
  ...
  const [paper, setPaper] = useState<Post | null>(null);
  ...
  useEffect(() => {
    const fetchPaper = async () => {
      ...
      const fullPath = window.location.pathname.replace('/papers/', '');
      if (!fullPath) {
      ...
      try {
        const response = await getPaperBySlug(fullPath);
        if (!response) {
  ...
  // Update meta tags when paper data is available
  useEffect(() => {
    if (paper) {
      document.title = `${paper.title} | iamsitting`;
      const metaDescription = document.querySelector('meta[name="description"]');
      if (metaDescription) {
        metaDescription.setAttribute('content', paper.description || 'Read this paper on iamsitting');
      }
    }
  }, [paper]);
    ...
    return (
      <div className="max-w-screen-lg mx-auto p-4">
        ...
        <div className="text-center">
          <Link to="/" className="text-[#4CAF50] hover:text-[#66BB6A] transition-colors">
            ← Back to Home
          </Link>
        </div>
      </div>
  ...
  if (!paper) {
    return (
      <div className="max-w-screen-lg mx-auto p-4">
        <div className="text-gray-500 text-center mb-4">Paper not found</div>
        <div className="text-center">
          <Link to="/" className="text-[#4CAF50] hover:text-[#66BB6A] transition-colors">
            ← Back to Home
          </Link>
        ...
        <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => {
                  const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(paper.title)}&url=${encodeURIComponent(window.location.href)}`;
            ...
            <div className="text-center space-y-4">
              <p className="text-gray-600 dark:text-gray-400">
                Have thoughts on this paper? Share them on{' '}
                <a 
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Thoughts on "${paper.title}" by @iamsitting: ${window.location.href}`)}`}
                  target="_blank"
